...
const preset = ref<PresetKeys>(layoutConfig.preset);
...
function getPresetExt() {
    const color: { name: string; [key: string]: any } | undefined = primaryColors.value.find((c) => c.name === layoutConfig.primary);
    if(color) {
        if (color.name === 'noir') {
            return {
                semantic: {
