...
if (route.params.id && userRole !== 'siswa') {
    siswa_id.value = Number(route.params.id)
} else {
    siswa_id.value = isUserSiswa(authStore.user) ? authStore.user.siswa_id : 0
}
...
const field = z.object({
    ...
    provinsi_id: z.number({message: "field tidak boleh kosong"}).int().positive(),
    kabupaten_id: z.number({message: "field tidak boleh kosong"}).int().positive(),
    kecamatan_id: z.number({message: "field tidak boleh kosong"}).int().positive(),
    desa_id: z.number().int({message: "field tidak boleh kosong"}).positive(),
    banjar_id: z.number({message: "field tidak boleh kosong"}).int().positive(),
    ...
    agama_id: z.number({message: "field tidak boleh kosong"}).int().positive(),
    kebutuhan_khusus: z.number({message: "field tidak boleh kosong"}).int().nonnegative().max(1),
    tempat_lahir: z.string().regex(/^[A-Za-z\s]+$/, "input hanya menerima huruf"),
    tanggal_lahir: z.date({message: "field tidak boleh kosong"}).transform((val) => new Date(val).toISOString()),
    ...
    nama_ayah: z.string().regex(/^[A-Za-z\s]+$/, "input hanya menerima huruf"),
    ...
    lintang: z.string({message: "field tidak boleh kosong"}).transform((val) => Number(val)),
    bujur: z.string({message: "field tidak boleh kosong"}).transform((val) => Number(val))
...
onMounted( async() => {
    try {
        ...
        const siswaData = await getSiswaById(siswa_id.value)
        if (siswaData) {
    ...
    confirm.require({
        ...
        accept: async () => {
            try {
                ...
                const response = await updateSiswaById(formData.siswa_id!, form.values as SiswaDTO)
                if(response === 200) {
                    const formValues = {
                        ...form.values,
                ...
                <div class="card flex flex-col gap-4">
                    ...
                    <Form v-slot="$form" :initialValues="formData" :resolver="resolver" @submit="handleSubmit" class="flex flex-wrap gap-4 sm:w-56 md:w-full" v-if="!isLoading">
                        ...
                        <div class="flex flex-col grow basis-1/6 gap-2">
                            ...
                            <Select v-model="formData.kabupaten_id" id="kabupaten_id" name="kabupaten_id" :options="kabupatenDropdown" optionLabel="kabupaten_nama" optionValue="kabupaten_id" placeholder="Pilih Kabupaten" disabled fluid />
                        ...
                        <div class="flex grow basis-[100%] gap-2 justify-end mt-4">
                            ...
                            <Button :disabled="isRoleAdminSMP" type="submit" label="Simpan" :fluid="false" class="w-[20%]"></Button>
                        </div>
                    </Form>
