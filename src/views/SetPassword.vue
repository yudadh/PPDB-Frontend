...
async function onSubmitPassword(form: SubmitEventForm) {
    try {
        const response = await updateUserPassword(form.values.password, route.query.token as string)
        if(response){
            toast.add({
                severity: 'success',
                summary: 'Berhasil',
                detail: 'Set Password Berhasil, Silahkan Login Kembali',
                life: 2000
            })
            setTimeout(() => {
                router.push('/login')
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Set Password Akun Anda!</div>
                        <span class="text-muted-color font-medium">Set Password Baru</span>
                    </div>

                    <div>
                        <Form v-slot="$form" :resolver="formResolver" :initialValues="initialValues" @submit="onSubmitPassword" class="flex flex-col gap-4">
                            <label for="password" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                            <Password id="password" v-model="initialValues.password" name="password" placeholder="Masukkan Password Baru" :toggleMask="true" class="mb-4" fluid :feedback="false"></Password>
                            <Message v-if="$form.password?.invalid" severity="error" size="small" variant="simple">{{ $form.password.error.message }}</Message>
                            <!-- <p v-if="countDownState > 0">{{ countDownState }}</p>
                            <p v-else>Coba Lagi</p> -->
                            <Button label="Simpan" class="w-full" type="submit"></Button>
                        </Form>
                    </div>
                </div>
            </div>
