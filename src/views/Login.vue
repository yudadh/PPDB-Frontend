...
async function login(form: FormSubmitEvent) {
    try {
        await authStore.login(form.values.username, form.values.password)
        const user = authStore.user
        if(user) {
            toast.add({ severity: 'success', summary: 'Login Berhasil', detail: 'Login berhasil, silahkan lanjut' , life: 800 })
            setTimeout(() => {
                switch (user.role) {
                  case 'siswa':
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Selamat Datang di PPDB!</div>
                        <span class="text-muted-color font-medium">Login untuk melanjutkan</span>
                    </div>

                    <div>
                        <Form v-slot="$form" :initialValues="initialValues" :resolver="resolver" @submit="login" class="flex flex-col">
                            <div class="mb-4">
                                <label for="email1" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Username</label>
                                <InputText id="email1" type="text" placeholder="Username" class="w-full md:w-[30rem]" v-model="initialValues.username" name="username" />
                                <Message v-if="$form.username?.invalid" severity="error" size="small" variant="simple">{{ $form.username.error?.message }}</Message>
                            </div>
                            
                            <div class="mb-6">
                                <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                                <Password id="password1" v-model="initialValues.password" name="password" placeholder="Password" :toggleMask="true" fluid :feedback="false"></Password>
                                <Message v-if="$form.password?.invalid" severity="error" size="small" variant="simple">{{ $form.password.error?.message }}</Message>
                            </div>
                            <div class="flex items-center justify-between mt-2 mb-6 gap-8">
                                <router-link to="/change-password"><span class="font-medium no-underline ml-2 text-center cursor-pointer text-primary">Lupa password?</span></router-link>
                            </div>
